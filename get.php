 ...
 * @return string|null The enriched config string or null if invalid.
 */
function processAndEnrichConfig(string $config, string $source, int $key, array &$ipInfoCache): ?string {
    static $configFields = [
    ...
    $newName = sprintf(
        '%s %s | %s | %s | @%s | %d',
        $flag, $countryCode, $encryptionStatus, $type, $source, $key
    );
    ...
    return str_replace('amp%3B', '', $encodedConfig);
}
...
echo "\n3. Processing configs for main aggregate files (max " . CONFIGS_FOR_MAIN_AGGREGATE . " per source)..." . PHP_EOL;
$ipInfoCache = [];
$finalOutput = [];
$locationBased = [];
...
foreach ($configsList as $source => $configs) {
    ...
    foreach ($configsToProcess as $key => $config) {
        if ($totalConfigsToProcess > 0) {
            print_progress(++$processedCount, $totalConfigsToProcess, 'Processing (Main):');
        }

        $enrichedConfig = processAndEnrichConfig($config, $source, ($key + $key_offset), $ipInfoCache);

        if ($enrichedConfig !== null) {
            $finalOutput[] = $enrichedConfig;
            // Extract country code from the enriched name for location-based grouping
            preg_match('/([A-Z]{2}) \|/', $enrichedConfig, $matches);
            $countryCode = $matches[1] ?? 'XX';
            $locationBased[$countryCode][] = $enrichedConfig;
        }
    }
}
...
echo "\n4.5. Generating channel-specific subscriptions (max " . CONFIGS_FOR_CHANNEL_SUBS . " configs)..." . PHP_EOL;
if (is_dir(CHANNEL_SUBS_DIR)) deleteFolder(CHANNEL_SUBS_DIR);
...
foreach ($configsList as $source => $configs) {
    ...
    foreach ($configsToProcess as $key => $config) {
        ...
        if ($enrichedConfig !== null) {
            $channelSpecificConfigs[] = $enrichedConfig;
